...
contract OtcBriber {
    ...
    event NewBribeOffer(
        ...
        uint numWeeks,
        uint id,
        uint requiredVeCrvAmount,
        address bribeToken,
        uint weeklyBribeAmount
    );
    event BribeClaimed(
        address indexed voter,
        address indexed briber,
        ...
        uint claimPeriod,
        uint id,
        uint votedVeCrvAmount,
        uint requiredVeCrvAmount,
        address bribeToken,
        uint weeklyBribeAmount
    );
    event BribeRetrieved(
        address indexed voter,
        address indexed briber,
        address indexed gauge,
        uint claimPeriod,
    ...
    function retrieveUnclaimedBribesPerWeek(uint256 id, uint256 week) external noReentry {
        ...
        require(
            canRetrieve || block.timestamp > claimingPeriod + WEEK * 4,
            "too close to claiming period"
        );
