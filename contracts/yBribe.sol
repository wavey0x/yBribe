...
contract yBribe {
    ...
    function rewards_per_gauge(
        address gauge
    ) external view returns (address[] memory) {
        return _rewards_per_gauge[gauge];
    }

    function gauges_per_reward(
        address reward
    ) external view returns (address[] memory) {
    ...
    function _update_period(
        ...
        if (block.timestamp >= _period + WEEK) {
            ...
            uint _amount = reward_per_gauge[gauge][reward_token] -
    ...
    function claim_reward_for_many(
        ...
        require(
            _users.length == _gauges.length &&
                _users.length == _reward_tokens.length,
            "!lengths"
        );
        uint length = _users.length;
        amounts = new uint[](length);
        for (uint i = 0; i < length; i++) {
            amounts[i] = _claim_reward(
                _users[i],
                _gauges[i],
                _reward_tokens[i]
            );
    ...
    function _claim_reward(
        address user,
        address gauge,
        address reward_token
    ) internal returns (uint) {
        if (
            blacklist.contains(user) || next_claim_time[user] > current_period()
        ) {
            return 0;
        }
        ...
        if (last_user_claim[user][gauge][reward_token] < _period) {
            ...
            if (GAUGE.last_user_vote(user, gauge) < _period) {
                ...
                uint _user_bias = _calc_bias(vs.slope, vs.end);
                _amount =
                    (_user_bias * reward_per_token[gauge][reward_token]) /
                    PRECISION;
                if (_amount > 0) {
