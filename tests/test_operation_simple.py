...
def test(gauge1):
    ...
    total_slope = (
        gauge_controller.points_weight(gauge, period).dict()["slope"] / decimals
    )
    y_slope = (
        gauge_controller.vote_user_slopes(yearn_voter, gauge).dict()["slope"] / decimals
    )
    c_slope = (
        gauge_controller.vote_user_slopes(convex_voter, gauge).dict()["slope"]
        / decimals
    )
