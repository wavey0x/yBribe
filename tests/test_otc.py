...
def test_standard_operation(
    token1, token2, token1_whale, bribe, user, otc,
    ...
    t1 = otc.setupBribe(voter2, gauge1, token1, 10_000*1e18, require_vecrv, 4, {'from': token1_whale})
    ...
    assert token1.balanceOf(otc) == offer['weeklyBribeAmount']*4 == offer1['weeklyBribeAmount'] * 4
...
def test_not_voted_enough(
    ...
    t1 = otc.setupBribe(voter2, gauge1, token1,weekly_bribe , require_vecrv, 4, {'from': token1_whale})
    offer = t1.events['NewBribeOffer']
