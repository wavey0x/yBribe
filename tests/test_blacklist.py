...
def test_blacklist_bias(
    ...
    user_bias3 = user_slope * (
        user_lock_end - bribe.current_period()
    )  # Here we use current_period instead of chain.time()
    # user_bias4 = user_slope * (user_lock_end - (bribe.current_period() - WEEK)) # Here we use current_period instead of chain.time()
    gauge_bias = gauge_controller.points_weight(gauge1, period_updated["period"])[
        "bias"
    ]
